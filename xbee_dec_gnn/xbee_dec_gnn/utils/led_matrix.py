...
class LEDMatrix:
    ...
    def set_square(self, color, size, top_left, color_space="rgb"):
        ...
        for i in range(self.config["height"]):
            for j in range(self.config["width"]):
                if i >= top_left[0] and i < top_left[0] + size and j >= top_left[1] and j < top_left[1] + size:
    ...
    def set_rainbow(self, brightness, start=0):
        pixels = [self.hsv2rgb(h, 255, brightness) for h in np.roll(np.linspace(0, 255, self.config["count"]), start)]
        self._set_pixels(pixels)
...
if __name__ == "__main__":
    ...
    print("Colormap")
    for i in range(10):
        color = LEDMatrix.from_colormap(i / 10, color_space="hsv", cmap_name="jet")
        color = (color[0], color[1], color[2] * 0.2)
        led.set_all(color=color, color_space="hsv")
        time.sleep(0.1)
    time.sleep(2)
...
class DummyLEDMatrix(LEDMatrix):
    def __init__(self):
        pass
